...
class ListenersCommand extends AbstractCommandStopwatchDecorator
{
    private const NAME = 'cmap:map:listeners';
     ...
     * @var ListenerQuery
     */
    private $listenersQuery;

    public function __construct(AstFactoryInterface $astFactory, ListenerQuery $listenerQuery)
    {
        parent::__construct();
        $this->astFactory = $astFactory;
        $this->listenersQuery = $listenerQuery;
    }

    protected function configure(): void
    {
        $this->setDescription('Create a full report about listeners.')
            ->addOption(
                self::OPT_FOLDER,
                'o',
    ...
    protected function executeUseCase(InputInterface $input, OutputInterface $output): void
    {
        try {
            ...
            $this->io->comment('Listener count: ' . count($map));
        } catch (Error $e) {
