...
class UseCasesCommand extends AbstractCommandStopwatchDecorator
{
    private const NAME = 'cmap:map:use-cases';
    ...
    public function __construct(AstFactoryInterface $astFactory, UseCaseQuery $useCaseQuery)
    {
        parent::__construct();
    ...
    protected function executeUseCase(InputInterface $input, OutputInterface $output): void
    {
        try {
            ...
            $map = $this->useCaseQuery->queryAst($ast);
            $map = $this->sort('FQCN', $map);
            $this->io->title('USE CASE PER BOUNDED CONTEXT');
            $this->io->section('Bounded context: ');
            $this->io->table(array_keys($map[0]), $map);
            $this->io->comment('Use case count: ' . count($map));
        } catch (Error $e) {
